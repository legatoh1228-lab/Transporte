...
import { Search, Filter, Plus, Eye, Edit2, IdCard, Users, ShieldAlert, CheckCircle, Clock } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <MetricCard title="Total Operadores" value="1,245" icon={Users} trend="+12" />
        <MetricCard title="Licencias Activas" value="1,080" icon={CheckCircle} color="text-tertiary-container" />
        <MetricCard title="Próximas a Vencer" value="124" icon={Clock} color="text-secondary-container" />
        <MetricCard title="Licencias Vencidas" value="41" icon={ShieldAlert} color="text-error" />
      </div>
...
function MetricCard({ title, value, icon: Icon, color = "text-primary", trend }: { title: string; value: string; icon: React.ElementType; color?: string; trend?: string }) {
  return (
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === 'Activo') {
    ...
    styles = "bg-[#ffb4ab]/20 text-[#93000a] border-[#ffb4ab]/30"; // Warning-ish color from error palette for demo
...
function OperatorModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
